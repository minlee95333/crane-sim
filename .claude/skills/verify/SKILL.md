...
P7.10: S12에서 작업자 5명 배회·지게차 순환 가시, 부재 픽업 후 대기하면 HUD에
`⛔ 지상 인원·장비 접근`과 안전 라인 `홀드 n회 …s`가 뜨고 홀드 중 조작이 안 먹는다
(시드 결정론 — 같은 리셋이면 같은 시점에 홀드). 지게차는 크레인 앞에서 대기.

P7.11 보조 오버레이: `.ov-label`이 권하 전 `수평 …·수직 …✕` → 내리면 `픽업 가능 — Space`
→ 매달면 `목표까지 …m`로 전환 (DOM 텍스트로 단정 가능). `.ov-banner`는 줄걸이 진행 %
→ S12 홀드 시 `⛔ 지상 인원·장비 접근`. H 키 → #overlay display none. 바람 위젯은
wind 있는 시나리오에서만 computed display=flex (S1 none/S11 flex/S12 none).
